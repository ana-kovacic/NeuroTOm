...
cmps <- readxl::read_xlsx("data/AnnotatedComp_tomatoProfile_filtered_ms2.xlsx")
...
excluded <- data.frame(
  rbind(
    ...
    c("Rutin", "POS"),
    c("Tryptamine", "NEG"),
    c("Amino(nitrophenyl)methylsulfonyl−propanoic acid", "POS"),
    c("Eriodictyol (I)", "NEG"),
    c("Eriodictyol (II)", "NEG"),
    c("Isoquercetin", "POS")
     )
) 
  ...
  for(p in c("POS", "NEG")){
    if(!paste(cmps$name[i], p) %in% paste(excluded$name, excluded$POL)) {
      ...
      if(cmps$level[i] == "L1"){
        ...
        if(!is.na(tmp)){
          sps2 <- read_excel("data/L2_MSMS.xlsx", sheet = tmp)
          yl <- c(-1.1, 1.1)
          sps2$intrel <- sps2$intensity / max(sps2$intensity)
