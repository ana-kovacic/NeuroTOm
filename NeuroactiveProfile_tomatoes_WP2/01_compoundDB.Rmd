...
```{r import}
if(file.exists(paste0("rdata/CompDb_tomato_", db, ".sqlite"))){
  file.remove(paste0("rdata/CompDb_tomato_", db, ".sqlite"))
}
if(db == "stds"){
  ...
  cmps <- read_xlsx("data/AnnotatedCompounds_AnnComp.xlsx")
} else if(db == "MS2_ann"){
...
db_file <- createCompDb(cmps, metadata = metad, path = "rdata/", 
                        dbFile = paste0("CompDb_tomato_", db, ".sqlite"))
```
