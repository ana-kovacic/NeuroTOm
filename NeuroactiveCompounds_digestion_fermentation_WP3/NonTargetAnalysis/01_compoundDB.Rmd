...
```{r, message=FALSE}
db <- "BA" # "stds" "SL" "MS2_ann" "Metabol_pathway" "BA" 
...
```{r import}
if(file.exists(paste0("rdata/CompDb_fecal_", db, ".sqlite"))){
  file.remove(paste0("rdata/CompDb_fecal_", db, ".sqlite"))
}
if(db == "Metabol_pathway"){
  cmps <- read.csv("data/Metabol_aspartame.csv")
} else if (db == "BA"){
  cmps <- read_xlsx("data/suspectlist/suspectList_BileAcid.xlsx")
} else if (db == "SL"){
  cmps <- read.csv("data/suspectlist/SuspectList_3rd_200223.csv")
} else if (db == "AnnComp"){
  cmps <- read_xlsx("data/AnnotatedCompounds_SL.xlsx")
} else if(db == "MS2_ann"){
  cmps <- read_xlsx("data/AnnotatedCompounds_MS2.xlsx")
}
...
```{r metad}
metad <- make_metadata(source = "fecal", url = "",
                       source_version = "", 
...
db_file <- createCompDb(cmps, metadata = metad, path = "rdata/", 
                        dbFile = paste0("CompDb_fecal_", db, ".sqlite"))
```
...
```{r save}
ionsdb <- ions(idb, c(
  "compound_id", "name", "ion_adduct", "ion_mz", "ion_rt", "comment", "activity"))
